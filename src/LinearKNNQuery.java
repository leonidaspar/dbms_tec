...
public class LinearKNNQuery extends LinearQuery {
    private ArrayList<Double> searchPoint;
    private int k;
    private PriorityQueue<HoldPairId> nearestNeighbours;


    LinearKNNQuery(ArrayList<Double> searchPoint, int k) {
        if (k < 0)
            throw new IllegalArgumentException("Parameter 'k' for the nearest neighbours must be a positive integer.");
        this.searchPoint = searchPoint;
        this.k = k;
        this.nearestNeighbours = new PriorityQueue<>(k, new Comparator<HoldPairId>() {
            @Override
            public int compare(HoldPairId recordDistancePairA, HoldPairId recordDistancePairB) {
    ...
    ArrayList<Long> getQueryRecordIds() {
        ...
        {
            HoldPairId recordDistancePair = nearestNeighbours.poll();
        ...
        while(blockId < DataHandler.getTotalBlocksInDataFile())
        {
            ...
            if (recordsInBlock != null)
            {
                ...
                while(i < entries.size()){
                    double distanceFromPoint = entries.get(i).getBoundingBox().findMinDistance(searchPoint);
                    if(nearestNeighbours.size() == k){
                        if(distanceFromPoint < nearestNeighbours.peek().getDistance()){
                            nearestNeighbours.poll();
                            nearestNeighbours.add(new HoldPairId(entries.get(i).getRecordId(), distanceFromPoint));
                        }
                    }else{
                        nearestNeighbours.add(new HoldPairId(entries.get(i).getRecordId(), distanceFromPoint));
                    }
